...
export function PlatformHeader({ sound, onSound, onBack, onRestart }: { sound: boolean; onSound: (value: boolean) => void; onBack?: () => void; onRestart?: () => void }) {
  return <header className="platform-header">
    {onBack ? <button className="brand-button" type="button" onClick={onBack} aria-label="Volver al catálogo"><span className="platform-mark" aria-hidden="true">II</span><span>{platform.name}</span></button>
      : <div className="brand-button"><span className="platform-mark" aria-hidden="true">II</span><span>{platform.name}</span></div>}
